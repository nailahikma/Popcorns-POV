...
<body class="font-poppins">
    ...
    <div class="mx-auto max-w-screen min-h-screen bg-black text-white md:px-10 px-3">
        ...
        <div class="py-24 flex laptopLg:ml-[680px] laptopXl:ml-[670px]">
            <div>
                ...
                <form class="w-[370px]" method="POST" action="{{ route('login') }}">
                    @csrf
                    <div class="flex flex-col gap-6">
                        <div>
                            <label class="text-base block mb-2">Email Address</label>
                            <input type="email" name="email"
                                class="rounded-2xl bg-form-bg py-[13px] px-7 w-full focus:outline-merah focus:outline-none @error('email') is-invalid @enderror" value="{{ old('email') }}"
                                placeholder="Email Address" />
                        </div>
                        <div>
                            <label class="text-base block mb-2">Password</label>
                            <input type="password" name="password"
                                class="rounded-2xl bg-form-bg py-[13px] px-7 w-full focus:outline-merah focus:outline-none @error('password') is-invalid @enderror" value="{{ old('password') }}"
                                placeholder="Password" />
                        </div>
                    </div>
                    <div class="grid space-y-[14px] mt-[30px]">
                        <button type="submit" class="rounded-2xl bg-red-900 py-[13px] text-center">
